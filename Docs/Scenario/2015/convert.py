...
def get_name_label(speaker):
    ...
    }.get(speaker.strip(), (speaker.strip(), speaker.strip()))
...
def process_followup_lines(lines):
    ...
    for line in blocks:
        if ":" in line:
            ...
            # ✅ 혹시 speaker_raw가 튜플형 문자열이면 파싱해서 첫 값만 사용하자고요.. 
            if speaker_raw.startswith("(") and speaker_raw.endswith(")"):
                try:
                    parsed = eval(speaker_raw)
                    if isinstance(parsed, tuple):
                        speaker_raw = parsed[0]
                except Exception:
                    pass  # 그냥 통과 ㄱㄱㄱ
